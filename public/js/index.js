...
function criaLista(){ //Função que cria novas listas
    ...
    <div class="lista" id="${listaDeListas[listaDeListas.length-1].id}">
        <div class="lista-header">
            ...
            <input class="tresPontinho" id="tresPontinho${listaDeListas[listaDeListas.length-1].id}" onclick="tresPontinho(this.id)" type="image"  src="{{ URL::asset('images/more.svg') }}" alt="more">
        </div>
        ...
        inputTarefa.addEventListener("keyup",function(event){
                ...
                if(event.key === "Enter"){
                    ...
                    if(descricaoTarefa){
                        ...
                        let text = `
                        <div class="tarefa">
                            <img id="lixo" class="lixeira" src="{{ URL::asset('images/thrash.svg') }}" alt="thrash">
...
function tresPontinho(idTarg){
    ...
    if(listaDeListas[objListaClicada.id].menuClicado===true){
        ...
        let text = `<div class="caixaMenu" id="caixaMenu">
            <input type="image" class="canetaMenu" id="canetaMenu${objListaClicada.id}" onclick="renomeiaLista(this.id)" src="{{ URL::asset('images/caneta.svg') }}">
            <button onclick="renomeiaLista(this.id)" class="menuTrocaNome" id="menuTrocaNome${objListaClicada.id}">Renomear lista</button>
            <input type="image" id="lixoMenu${objListaClicada.id}" onclick="deletaLista(this.id)" class="lixeiraMenu" src="{{ URL::asset('images/thrash.svg') }}">
